...
- Add a `migrate-to-agent-manager.sh` script to this repo
  ...
  - What: a single portable, idempotent bash script (VPS, Ubuntu, and Mac installs) that repoints the git remote at `okthink-ai/agent-manager` (preserving SSH vs HTTPS form), stops the server, renames the install directory to `agent-manager`, then delegates update + rebuild + restart + verification to `migrate-to-expo.sh --dir <new-path>` — the established idempotent updater with a clean-tree guard, rollback messaging, and reachability preservation.
...
2. **Remote** — `git remote set-url origin <new URL>`; echo old → new. This must precede the delegation: the updater pulls with `git pull --ff-only origin main` (migrate-to-expo.sh:459), and the point is for that pull to ride the new URL, not the redirect.
...
6. **Summary** — new path, new remote, dashboard URL; reminders that shell history, personal scripts, SSH sessions, and running agent sessions reference the old path; rollback recipe (mv back, set-url back, restart).
